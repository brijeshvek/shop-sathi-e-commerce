...
// Database connection string from .env or hardcoded for seeding
const MONGO_URI = 'mongodb+srv://brijeshvek:W4V1U7D72Kj3FOPu@cluster0.v28s6.mongodb.net/ShopEase?retryWrites=true&w=majority&appName=Cluster0'
...
async function seedCategories() {
  try {
    ...
    // Be careful, this wipes existing categories
    // await Category.deleteMany({})

    for (const mainCat of categoriesData) {
      console.log(`Processing main category: ${mainCat.name}`)
      // Create or find main category
      let parentDoc = await Category.findOne({ name: mainCat.name })
      if (!parentDoc) {
        parentDoc = await Category.create({ 
          name: mainCat.name, 
          description: mainCat.description || '' 
        })
        console.log(`  -> Created ${mainCat.name}`)
      }

      if (mainCat.children && mainCat.children.length > 0) {
        for (const subCat of mainCat.children) {
          let subDoc = await Category.findOne({ name: subCat.name })
          if (!subDoc) {
            ...
            // Update parent if it was wrong
            if (subDoc.parent?.toString() !== parentDoc._id.toString()) {
              subDoc.parent = parentDoc._id
              await subDoc.save()
            }
          }
          ...
          if (subCat.children && subCat.children.length > 0) {
            for (const subSubCat of subCat.children) {
              let subSubDoc = await Category.findOne({ name: subSubCat.name })
              if (!subSubDoc) {
                ...
                console.log(`      -> Created ${subSubCat.name}`)
              }
            }
          }
        }
