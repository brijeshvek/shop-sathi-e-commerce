...
export function RecommendedProducts() {
  ...
  useEffect(() => {
    const fetchRecommended = async () => {
      try {
        // Mock recommendation: Fetch recent items and take a slice to simulate recommendations
        const { data } = await api.get('/products?limit=8');
        setProducts(data.data?.slice(4, 8) || data.data || []);
      } catch {
        // Silently fail
      } finally {
        setIsLoading(false);
  ...
  return (
    <section className="py-8 sm:py-12" aria-label="Recommended Products">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-5 sm:mb-8">
          <div className="flex items-center space-x-2">
            <ThumbsUp className="w-6 h-6 sm:w-8 sm:h-8 text-primary-500" aria-hidden="true" />
