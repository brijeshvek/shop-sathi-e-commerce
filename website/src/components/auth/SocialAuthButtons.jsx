...
export function SocialAuthButtons({ mode = "login" }) {
  ...
  const handleSocialButtonClick = async (provider) => {
    // If Firebase is configured with real project keys, trigger authentic OAuth Popup!
    if (isFirebaseConfigured()) {
      ...
      try {
        ...
        if (err?.code !== "auth/popup-closed-by-user" && err?.code !== "auth/cancelled-popup-request") {
          console.log("Firebase popup error, opening standard dialog:", err.message);
        } else {
    ...
    // Otherwise open standard direct login dialog
    openAccountPicker(provider);
  };
    ...
    <div className="w-full space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <button
          ...
          disabled={loadingProvider !== null}
          className="flex items-center justify-center py-2.5 px-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-750 text-gray-700 dark:text-gray-200 font-medium text-xs sm:text-sm shadow-sm hover:shadow transition-all group focus:outline-none focus:ring-2 focus:ring-gray-900 dark:focus:ring-white active:scale-95 disabled:opacity-60"
